...
"""Generate everything that derives from module manifests, or check that it is current.
    ...
    tools/manifest.py generate     rewrite runtime/generated/*, python/lemmakernel/_manifest.py
...
producer of the C++ registry data, the runtime description JSON, the CMake source list and the
...
REQUIRED_FILES = ("contract", "reference", "naive", "bench", "tests")
FEATURE_FLAGS = {
...
def outputs(modules: list[dict]) -> dict[Path, str]:
    return {GENERATED_CPP: render_cpp(modules), GENERATED_CMAKE: render_cmake(modules), GENERATED_PY: render_python(modules)}
