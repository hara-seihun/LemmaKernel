...
/-- Every element of the matrix group generated by invertible `n × n` generators, once and in
lexicographic order of flattened entries. -/
theorem matrixElements_spec (gens : List Mat) (n : ℕ)
    (h : ∀ g ∈ gens, WellFormed p n n g ∧ IsUnit (toMatrix p n n g)) :
    (matrixElements p gens).Nodup := by
  sorry
