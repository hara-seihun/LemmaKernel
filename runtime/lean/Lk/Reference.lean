...
/-- Every element of the matrix group generated by `gens`, lexicographic by flattened entries. -/
def matrixElements (p : Nat) (gens : List Mat) : List Mat :=
  let n := (gens.headD []).length
  sortBy (fun a b => lexLe a.flatten b.flatten)
...
inductive Family
  ...
  | groupElements (p : Nat) (gens : List Mat)
...
def Family.members : Family → List Mat
  ...
  | .groupElements p gens =>
      if p = 0 then (permElements (gens.map (·.headD []))).map ([·]) else matrixElements p gens
