/* heat_dirichlet: the fixed-point arithmetic, the request parameters and the phase-aware
 * bound's setup, shared by the generic (CPU) and hip (GPU) backends.
 *
 ...
 * coefficient enclosures; a member is a theta box of the grid, and its value is the minimum over
 * psi samples of the bound from the centre value, the gradient and the second-order remainder of
 * each component over the box, with terms whose arc exceeds a radian enclosed by their rectangle
 * instead, minus the losses.
 */
#pragma once
...
inline int bit_length(unsigned __int128 n) {
    int b = 0;
    while (n) { ++b; n >>= 1; }
    return b;
}

inline I ln_mantissa(I m, bool lower) {
    I num = (m - S) * S, den = m + S;
...
inline Iv ln_bounds(unsigned __int128 n) {
...
inline std::optional<I> exp_upper(I x) {
...
inline std::optional<I> exp_lower(I x) {
    if (x > EXP_LIMIT) return std::nullopt;
...
inline Iv iadd(Iv a, Iv b) { return {a.lo + b.lo, a.hi + b.hi}; }
inline Iv isub(Iv a, Iv b) { return {a.lo - b.hi, a.hi - b.lo}; }
inline Iv iscale(Iv a, I num, I den) {
    I p0 = a.lo * num, p1 = a.hi * num;
    return {fdiv(std::min(p0, p1), den), cdiv(std::max(p0, p1), den)};
}
inline std::optional<Iv> iexp(Iv a) {
    auto lo = exp_lower(a.lo), hi = exp_upper(a.hi);
    if (!lo || !hi) return std::nullopt;
    return Iv{*lo, *hi};
}
inline I abs_upper(Iv a) { return std::max(iabs(a.lo), iabs(a.hi)); }
...
inline I sqrt_lower(I a) { return (I)isqrt128((unsigned __int128)(a * S)); }
inline I sqrt_upper(I a) {
...
inline I abs_lower(Iv a) { return (a.lo <= 0 && 0 <= a.hi) ? 0 : std::min(iabs(a.lo), iabs(a.hi)); }

/* x^n / n! at scale S for x >= 0, rounded one way. */
inline I powfact(I x, int n, bool lower) {
    I term = S;
...
inline Iv ineg(Iv a) { return {-a.hi, -a.lo}; }
...
inline Cv unit_circle(uint64_t M, uint64_t J) {
